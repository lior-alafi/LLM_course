...
class ContextSummaryStore:
    ...
    def set_summary(self, session_id:str, summary:str)->None:
        with self.lock_path.open('r+') as lf:
            ...
            try:
                data=self._read(); data[session_id]={'summary':summary}; self._write(data)
            finally: fcntl.flock(lf.fileno(), fcntl.LOCK_UN)
    def _read(self)->dict:
        try:
...
class ContextSummaryService:
    ...
    def maybe_update_summary(self, *, session_id:str, records:list[InteractionRecord], threshold:int, recent_keep:int)->None:
        ...
        try:
            raw=self.llm.complete_text(messages); data=extract_json_object(raw); result=ContextSummaryResult.model_validate(data)
            ...
            self.store.set_summary(session_id,result.summary)
        except Exception: return
