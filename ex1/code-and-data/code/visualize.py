...
def plot_attention_maps(
    attn_maps,          # (B, n_layers, n_heads, N, N) tensor
    tokens: list[str],
    sample_idx: int = 0,
    max_len: int = 32,
    save_path: str = None,
    ...
    fig.suptitle("Attention Maps (layer × head)", fontsize=14, y=1.01)

    for layer_idx in range(n_layers):
        for head_idx in range(n_heads):
            attn = attn_maps[sample_idx, layer_idx, head_idx, :seq_len, :seq_len].detach().cpu()
